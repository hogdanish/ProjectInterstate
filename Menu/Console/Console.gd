...
class_name Console extends CanvasLayer
...
const _floating_widget_pck = preload("res://Menu/Console/components/floating_widget/floating_widget.tscn")

var _envs = {}
var _envs_info = {}
var _expression = Expression.new()
var _config = ConsoleConfig.get_config()
var _current_hints := {}
var _hint_idx := 0
...
#	notify("[color=green][Info][/color] [b]%s[/b] env loaded!"%env_name)
	...
	if env.get_script():
		var env_info = ConsoleUtils.extract_info_from_script(env.get_script())
		for k in env_info:
...
func load_widgets_plan(plan:String):
	...
	notify("Press the tilde key (~) to open the console!")
...
func WindowsBoot():
	output("Michaelsoft Binbows [Version 10.0.022623.1037]")
	output("(c) Michaelsoft Corporation. No rights reserved.")

func MacBoot():
	output("SSH connection has been successfully established with host")
	output("For more information, please visit https://support.brapple.com/sp/134213")

func LinuxBoot():
	output("Running Gnignix Kernel 3.2.14.9")
	output("Shell commands are defined internally. Type 'help' to see a list.")
...
func ChooseKey():
	pass
#	if Globals.unix_based_os.has(Globals.os_name):
#		Globals.key = ":~$"
#	if Globals.windows_os.has(Globals.os_name):
#		Globals.key = ">"
...
func _ready():
	...
	output("[color=burlywood][b][color=burlywood]PankuConsole Version 1.1.23[/color][/b][/color]")
	output("[color=burlywood][b]Check [color=chartreuse]default_env.gd[/color] or simply type [color=green]help[/color] to see what you can do now![/b][/color]")
	...
	_input_area.update_hints.connect(
		func(exp:String):
			_current_hints = ConsoleUtils.parse_exp(_envs_info, exp)
	...
	var env_info = ConsoleUtils.extract_info_from_script(_base_instance.get_script())
...
func _notification(what):
	...
	if what == 1006:
		#save back config
		save_current_widgets_as(_config["widgets_system"]["current_plan"])
		ConsoleConfig.set_config(_config)
