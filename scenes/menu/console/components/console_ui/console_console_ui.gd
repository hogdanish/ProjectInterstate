...
func _ready():
	output("[b][color=burlywood][ Console Console ][/color][/b]")
	...
	output("[color=burlywood][b]For more info, please visit: [color=green][url=https://github.com/Ark2000/ConsoleConsole]project github page[/url][/color][/b][/color]")
	output("")
	...
	_input_area.submitted.connect(
		func(exp:String):
			...
			if exp.is_empty():
				return
			var result = Globals.Console.execute(exp)
			output(exp)
			if !result["failed"]:
	...
	_input_area.update_hints.connect(
		func(exp:String):
			_current_hints = ConsoleUtils.parse_exp(Console._envs_info, exp)
